...
class UserRequestSerice {
  ...
  async getAll(): Promise<IUserRequest[]> {
    try {
      const usersrequests = await UserRequest.find().select("-senhaTemporaria");
      return usersrequests;
    } catch (error) {
      console.error(
        "Erro ao buscar todas as solicitações de criação de usuários: ",
        error
      );
  ...
  async getAllByDepartment(departamento: string): Promise<IUserRequest[]> {
    return await UserRequest.find({ departamento });
  }
  ...
  async create(data: CreateUserRequestDTO): Promise<IUserRequest> {
    try {
      ...
      if (existingUserRequest) {
        throw new Error(
          "Já existe uma solicitação pendente ou aprovada com este e-mail."
        );
      }
      ...
      console.error(
        "Erro na criação de uma nova solicitação de criação de usuário:",
        error
      );
      throw error;
    }
  ...
  async deleteRequest(id: string, userId: string): Promise<boolean> {
    try {
      const req = await UserRequest.findOne({
        _id: id,
        criadoPor: userId,
        status: "pendente",
      });
